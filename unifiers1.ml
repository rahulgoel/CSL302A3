...
let  rec subst ( ( t : preterm ), s) = 
	...
	| _ ->  t
exception E
let rec apply sub listterms outl = match listterms with	| [] -> outl
						| hdlist::restlist-> apply sub restlist outl@[subst(hdlist ,sub)]						

let rec applylist sublist listterms = match sublist with | [] -> listterms
...
let rec mgu t1 t2 listu =
	let rec isx s1 x=
	 	let rec chklist list ( str1 : string )  = match list with
			...
			match x with		
							| X ( S y) -> if (s1=y) then true 										else false
							| X ( I w) -> false
	...
	match t2 with 
	| X (I i) ->(match t1 with 
							| X ( I j) -> if (i =j) then listu@[( "I dont care" ,X( I j) )]
									else  raise E
							| X ( S x) -> listu@[(x,X (I i))]
							| Node(str,list) -> raise E)
	| X ( S s) -> (match t1 with
							| X ( I j ) -> listu@[(s,X (I j) )]
							| X ( S x1) -> listu@[(x1, X ( S s) )]
							| Node(str,list) ->if ( (isx s t1) = false)  then listu@[(s, Node(str,list))]	 									else raise E )
	| Node(str,list) -> match t1 with																			
	         | X ( I j) -> raise E
		| X ( S x1) -> if ( (isx x1 t2) = false)  then listu@[( x1,Node(str,list))]	
					else raise E
		|	Node(str1,list1) -> if(str1=str && (List.length list= List.length list1) ) then 
																										...
																										| hd::rest->(match  list1 with
																																		|hd2::res2->  listu@(mgu hd hd2 [])@(mgulist (applylist (mgu hd hd2 []) rest) (applylist (mgu hd hd2 []) res2) [])))
